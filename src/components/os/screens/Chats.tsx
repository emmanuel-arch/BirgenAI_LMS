...
export function ChatsScreen({
  ...
  const Item = ({ t, i }: { t: ThreadSummary; i: number }) => (
    ...
    >
      <button
        onClick={() => onOpen(t)}
        className="flex w-full items-start gap-2.5 rounded-2xl border border-zinc-900/[0.07] bg-white/75 px-3 py-2.5 text-left transition-all hover:border-[color:var(--brand)] hover:bg-white active:scale-[0.985]"
      >
        <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-zinc-900/[0.05] text-zinc-500">
        ...
        <span className="min-w-0 flex-1">
          <span className="flex items-center gap-1">
            ...
            <span className="min-w-0 flex-1 truncate text-[12.5px] font-semibold leading-tight text-zinc-800">{t.title}</span>
          </span>
          {t.preview && (
            <span className="mt-0.5 block line-clamp-2 text-[10.5px] leading-snug text-zinc-500">{t.preview}</span>
          )}
          <span className="mt-1 flex items-center gap-1.5 text-[9.5px] text-zinc-400">
      ...
      <div className="absolute right-2 top-2 flex gap-0.5 opacity-60 transition-opacity group-hover:opacity-100">
        <button
          ...
          className="flex h-6 w-6 items-center justify-center rounded-lg bg-white/80 text-zinc-400 hover:text-zinc-800"
        ...
        <button
          ...
          className={`flex h-6 items-center justify-center rounded-lg px-1.5 text-[9px] font-bold transition-colors ${
            confirming === t.id ? "bg-rose-500 text-white" : "bg-white/80 text-zinc-400 hover:text-rose-600"
          }`}
  ...
  return (
    <Screen>
      <div className="shrink-0 pb-2 pt-1">
        <div className="flex items-center gap-2 rounded-xl border border-zinc-900/[0.12] bg-white px-2.5 focus-within:border-[color:var(--brand)]">
          <Search className="h-3.5 w-3.5 shrink-0 text-zinc-400" />
          <input
            value={q}
            onChange={(e) => setQ(e.target.value)}
            placeholder="Search your conversations"
            className="flex-1 bg-transparent py-2 text-[12px] outline-none placeholder:text-zinc-400"
          />
        </div>
      </div>
      ...
      <div className="min-h-0 flex-1 space-y-1.5 overflow-y-auto pb-3">
        ...
        {shown.length === 0 && (
          <p className="pt-6 text-center text-[11.5px] text-zinc-500">Nothing matches &ldquo;{q}&rdquo;.</p>
        )}
      </div>
    </Screen>
  );
