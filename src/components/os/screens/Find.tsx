  ...
  return (
    <Screen>
      <div className="shrink-0 pb-2 pt-1">
        <div className="flex items-center gap-2 rounded-xl border border-ash-900/[0.12] bg-paper px-3 focus-within:border-[color:var(--brand)]">
          ...
          />
          {busy && <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin text-ash-400" />}
        </div>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto pb-3">
        ...
        {res && res.matches.length === 0 && !busy && (
          <div className="rounded-2xl border border-ash-900/[0.07] bg-paper/70 px-3.5 py-4 text-center">
            <p className="text-[12.5px] font-semibold text-ash-700">No one by &ldquo;{res.query}&rdquo;</p>
            <p className="mt-1 text-[11px] leading-snug text-ash-500">{scopeNote} Try a phone number or national ID — those match exactly.</p>
          </div>
        )}

        {res && res.matches.length > 0 && (
          <>
            <p className="mb-1.5 text-[10px] text-ash-500">
            ...
            <div className="space-y-1.5">
              {res.matches.map((m, i) => (
                <motion.button
                  ...
                  onClick={() => onOpen(m)}
                  className="flex w-full items-center gap-2.5 rounded-2xl border border-ash-900/[0.07] bg-paper/75 px-3 py-2.5 text-left transition-all hover:border-[color:var(--brand)] hover:bg-paper active:scale-[0.985]"
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[12.5px] font-semibold leading-tight text-ash-800">{m.name}</span>
                    <span className="mt-0.5 flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[10px] leading-tight text-ash-500">
                      <span>{m.phoneMasked}</span>
                      {m.nationalIdMasked && <><span className="text-ash-300">·</span><span>ID {m.nationalIdMasked}</span></>}
                      {m.branch && <><span className="text-ash-300">·</span><span className="inline-flex items-center gap-0.5"><Building2 className="h-2.5 w-2.5" />{m.branch}</span></>}
                    </span>
                    <span className="mt-1 flex flex-wrap items-center gap-1">
                      ...
                      {m.openLoans > 0 && (
                        <span className="rounded bg-ash-900/5 px-1.5 py-px text-[9px] font-medium text-ash-600">{m.openLoans} open</span>
                      )}
                      ...
                      {m.kycStatus !== "VERIFIED" && (
                        <span className="rounded bg-ash-900/5 px-1.5 py-px text-[9px] font-medium text-ash-500">KYC {m.kycStatus.toLowerCase()}</span>
                      )}
                    </span>
                  </span>
            ...
            {res.total > res.matches.length && (
              <p className="mt-2 text-center text-[10px] text-ash-400">
        ...
        {!res && !error && (
          <div className="mt-6 space-y-2 text-center">
            <UserSearch className="mx-auto h-8 w-8 text-ash-300" />
            <p className="text-[11.5px] leading-snug text-ash-500">
