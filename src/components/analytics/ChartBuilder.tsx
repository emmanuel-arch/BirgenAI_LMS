...
export default function ChartBuilder({
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[300px_minmax(0,1fr)]">
      {/* ── The column picker ────────────────────────────────────────────── */}
      <aside className="rounded-2xl border border-zinc-900/10 bg-white">
        <div className="flex items-center justify-between border-b border-zinc-900/[0.07] px-4 py-3">
          <div>
            <p className="text-[12px] font-bold text-zinc-800">Columns</p>
            <p className="text-[10px] text-zinc-500">{picked.length} selected</p>
          </div>
          {picked.length > 0 && (
            <button
              type="button"
              onClick={() => go({ f: null, form: null })}
              className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-[10px] font-semibold text-zinc-500 hover:bg-zinc-900/5"
            >
        ...
        <div className="max-h-[70vh] overflow-y-auto p-2">
          {fieldsBySource().map(({ source, fields }) => (
            <div key={source} className="mb-2">
              <p className="px-2 pb-1 pt-2 text-[9px] font-bold uppercase tracking-[0.12em] text-zinc-400">
                {SOURCE_LABEL[source]}
              </p>
              <p className="px-2 pb-1.5 text-[10px] leading-snug text-zinc-400">{SOURCE_BLURB[source]}</p>
              {fields.map((f) => {
                ...
                return (
                  <button
                    ...
                    className={`mb-0.5 flex w-full items-start gap-2 rounded-lg px-2 py-1.5 text-left transition-colors ${
                      on ? "bg-violet-50 ring-1 ring-violet-200" : "hover:bg-zinc-900/[0.04]"
                    }`}
                  >
                    <span
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="flex items-center gap-1">
                        <Icon className="h-2.5 w-2.5 shrink-0 text-zinc-400" />
                        <span className={`truncate text-[12px] ${on ? "font-semibold text-violet-900" : "text-zinc-700"}`}>
                          {f.label}
                        </span>
                      </span>
                      <span className="mt-0.5 block truncate text-[10px] leading-snug text-zinc-400">{f.hint}</span>
                    </span>
                  </button>
                );
              })}
        ...
        <div className="border-t border-zinc-900/[0.07] px-4 py-2.5">
          {(Object.keys(ROLE_ICON) as Array<keyof typeof ROLE_ICON>).map((r) => {
            const Icon = ROLE_ICON[r];
            return (
              <p key={r} className="flex items-center gap-1.5 py-0.5 text-[10px] text-zinc-500">
                <Icon className="h-2.5 w-2.5 text-zinc-400" /> {ROLE_HINT[r]}
              </p>
            );
          })}
        </div>
      ...
      <div className="min-w-0 space-y-3">
        ...
        {activeForm && dims.length === 1 && measures.length > 0 ? (
          ...
          <div className="flex h-56 items-center justify-center rounded-2xl border border-dashed border-zinc-900/15 bg-white px-6 text-center">
            <p className="max-w-sm text-[13px] leading-snug text-zinc-500">
              {dims.length === 0
        ...
        {picked.length > 0 && (
          <div className="rounded-2xl border border-zinc-900/10 bg-white p-4">
            <p className="text-[12px] font-bold text-zinc-800">Chart types</p>
                ...
                return (
                  <button
                    ...
                    className={`rounded-xl border p-3 text-left transition-colors ${
                      on ? "border-violet-400 bg-violet-50" : "border-zinc-900/10 hover:border-zinc-900/25"
                    }`}
                  >
                    <p className="flex items-center gap-1.5 text-[12px] font-semibold text-zinc-800">
                      {v.label}
                      {v.recommended && (
                    ...
                    <p className="mt-0.5 text-[11px] leading-snug text-zinc-500">{v.purpose}</p>
                  </button>
                );
              })}
            </div>
            ...
            {blocked.length > 0 && (
              <div className="mt-3 border-t border-zinc-900/[0.07] pt-3">
                <p className="text-[10px] font-bold uppercase tracking-wide text-zinc-400">Not available, and why</p>
                <div className="mt-1.5 space-y-1">
                  {blocked.map((v) => (
                    <p key={v.form} className="flex items-start gap-1.5 text-[11px] leading-snug text-zinc-500">
                      <Ban className="mt-0.5 h-3 w-3 shrink-0 text-zinc-300" />
                      <span>
                        <span className="font-semibold text-zinc-600">{v.label}</span> — {v.reason}
                      </span>
                    </p>
                  ))}
                </div>
        ...
        {picked.length === 0 && (
          <div className="rounded-2xl border border-zinc-900/10 bg-white p-4">
            <p className="text-[12px] font-bold text-zinc-800">Start from a question</p>
            <div className="mt-2 grid gap-2 sm:grid-cols-2">
              {QUICK_STARTS.map((q) => (
                <button
                  ...
                  className="rounded-xl border border-zinc-900/10 p-3 text-left transition-colors hover:border-violet-300 hover:bg-violet-50/50"
                >
                  <p className="text-[12px] font-semibold text-zinc-800">{q.label}</p>
