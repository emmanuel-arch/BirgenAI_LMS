    ...
    <div className="mx-auto max-w-[1400px] px-4 py-5 sm:px-6 sm:py-6">
      <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-xl font-bold tracking-tight text-ash-900 sm:text-2xl">{title}</h1>
          <p className="mt-0.5 max-w-2xl text-[13px] leading-snug text-ash-500">{blurb}</p>
        </div>
...
export function Band({ label, hint }: { label: string; hint?: string }) {
  return (
    <div className="mb-3 mt-6 flex items-baseline gap-3">
      <h2 className="text-[11px] font-bold uppercase tracking-[0.14em] text-ash-500">{label}</h2>
      {hint && <p className="truncate text-[11px] text-ash-400">{hint}</p>}
      <span className="h-px flex-1 bg-ash-900/[0.08]" aria-hidden />
    </div>
  );
}
