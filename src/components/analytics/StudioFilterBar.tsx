...
export default function StudioFilterBar({
  ...
  return (
    <div className={`sticky top-14 z-10 -mx-4 mb-4 border-b border-zinc-900/[0.07] bg-[#f6f6f4]/90 px-4 py-2.5 backdrop-blur sm:-mx-6 sm:px-6 ${pending ? "opacity-70" : ""}`}>
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          <div className="w-64 p-1.5">
            {(["recent", "calendar", "all"] as const).map((group) => (
              <div key={group}>
                <p className="px-2 pb-0.5 pt-2 text-[9px] font-bold uppercase tracking-wider text-zinc-400">
                ...
                {RANGE_DEFS.filter((r) => r.group === group).map((r) => (
                  <button
                    ...
                    className={`flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left text-[12px] transition-colors ${
                      range === r.key ? "bg-zinc-900 font-semibold text-white" : "text-zinc-700 hover:bg-zinc-900/5"
                    }`}
                  >
                    {r.label}
            ...
            <div className="mt-2 border-t border-zinc-900/[0.07] px-2 pt-2">
              <p className="pb-1 text-[9px] font-bold uppercase tracking-wider text-zinc-400">Custom</p>
              <div className="flex items-center gap-1.5">
                <input
                  type="date"
                  defaultValue={current.get("from") ?? ""}
                  onChange={(e) => go({ range: "custom", from: e.target.value })}
                  className="w-full rounded-md border border-zinc-900/10 px-1.5 py-1 text-[11px]"
                />
          ...
          >
            <div className="w-48 p-1.5">
              <button
                ...
                className={`flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left text-[12px] ${!grain ? "bg-zinc-900 font-semibold text-white" : "text-zinc-700 hover:bg-zinc-900/5"}`}
              >
              ...
              {GRAINS.map((g) => (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left text-[12px] ${grain === g.key ? "bg-zinc-900 font-semibold text-white" : "text-zinc-700 hover:bg-zinc-900/5"}`}
                >
                  {g.label} {grain === g.key && <Check className="h-3 w-3" />}
                </button>
              ))}
              <p className="px-2 pb-1 pt-2 text-[10px] leading-snug text-zinc-400">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1 rounded-lg px-2 py-1.5 text-[11px] font-medium text-zinc-500 hover:bg-zinc-900/5 hover:text-zinc-700"
          >
    ...
    <div className="relative">
      <button
        ...
        className={`inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px] transition-colors ${
          open ? "bg-zinc-900 text-white" : "bg-white text-zinc-600 ring-1 ring-zinc-900/10 hover:bg-zinc-900/[0.04]"
        }`}
      >
        {trigger}
      </button>
      {open && (
        <>
          ...
          <div id={`pop-${id}`} className="absolute left-0 top-full z-30 mt-1 max-h-[70vh] overflow-auto rounded-xl border border-zinc-900/10 bg-white shadow-xl">
            {children}
          </div>
        </>
      )}
  ...
  return (
    ...
    >
      <div className="w-64">
        ...
        {options.length > 8 && (
          <div className="border-b border-zinc-900/[0.07] p-2">
            <input
              autoFocus
              value={q}
              onChange={(e) => setQ(e.target.value)}
              placeholder={`Search ${label.toLowerCase()}…`}
              className="w-full rounded-md border border-zinc-900/10 px-2 py-1.5 text-[12px] outline-none focus:ring-2 focus:ring-zinc-900/20"
            />
          </div>
        )}
        <div className="max-h-64 overflow-auto p-1.5">
          {shown.length === 0 && <p className="px-2 py-3 text-center text-[11px] text-zinc-400">Nothing matches.</p>}
          {shown.map((o) => {
            ...
            return (
              <button
                ...
                className="flex w-full items-start gap-2 rounded-md px-2 py-1.5 text-left transition-colors hover:bg-zinc-900/5"
              >
                <span className={`mt-0.5 flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded border ${on ? "border-zinc-900 bg-zinc-900" : "border-zinc-300"}`}>
                  {on && <Check className="h-2.5 w-2.5 text-white" />}
                </span>
                <span className="min-w-0">
                  <span className="block truncate text-[12px] text-zinc-700">{o.label}</span>
                  {o.hint && <span className="block truncate text-[10px] text-zinc-400">{o.hint}</span>}
                </span>
              </button>
            );
          })}
        ...
        {chosen > 0 && (
          <div className="border-t border-zinc-900/[0.07] p-1.5">
