    ...
    <div className="mt-4 rounded-xl border border-ash-900/10 bg-paper/70 p-4">
      <div className="flex items-center gap-2">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-ash-500 leading-tight">See your score, and what a lender sees.</p>
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === "paying" && (
          <motion.div key="paying" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-4 text-center">
            ...
            <p className="mt-0.5 text-[11px] text-ash-500">{simulated ? "Just a moment." : `Enter your M-Pesa PIN to pay ${kes(price)}.`}</p>
          </motion.div>
        )}
        ...
        {phase === "ready" && (
          <motion.div key="ready" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-3">
            ...
            <label className="mt-3 flex cursor-pointer items-center gap-2 rounded-lg border border-dashed border-ash-900/20 bg-paper/60 px-3 py-3 text-[12px] hover:bg-paper">
              <Upload className="h-4 w-4 shrink-0 text-ash-400" />
              <span className="min-w-0 flex-1 truncate text-ash-600">{file ? file.name : "Attach your M-Pesa statement (PDF)"}</span>
            ...
            {needPassword && (
              <div className="mt-2 flex items-center gap-2 rounded-lg border border-ash-900/15 bg-paper/80 px-3">
                ...
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
                  placeholder="Statement password" className="flex-1 bg-transparent py-2.5 text-sm outline-none placeholder:text-ash-400" />
              </div>
            )}
        ...
        {phase === "crunching" && (
          <motion.div key="crunching" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-4 text-center">
            ...
            <p className="mt-0.5 text-[11px] text-ash-500">Clustering merchants, scoring your habits.</p>
          </motion.div>
        )}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-xl p-4 text-center" style={{ background: `linear-gradient(135deg, ${tone.soft}, transparent)` }}>
        <p className="text-[10px] font-bold uppercase tracking-wider text-ash-400">Internal Score</p>
        <p className="text-4xl font-black tabular-nums" style={{ color: tone.color }}>{report.score.value}</p>
        <p className="text-[11px] text-ash-500">of 900</p>
        ...
        <div className="space-y-1">
          {drivers.map((d, i) => (
            <div key={i} className="flex items-start gap-2 text-[11px]">
              ...
              <span className="text-ash-600"><strong className="text-ash-800">{d.factor}</strong> — {d.detail}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {topCats.length > 0 && (
        <div>
          <p className="mb-1.5 text-[10px] font-bold uppercase tracking-wider text-ash-400">Where your money goes</p>
          <div className="space-y-1.5">
            {topCats.map((c, i) => (
              <div key={c.category}>
                <div className="flex items-center justify-between text-[11px]">
                  <span className="text-ash-600">{prettyCat(c.category)}</span>
                  <span className="font-semibold tabular-nums text-ash-700">{kes(c.amount)}</span>
                </div>
                <div className="mt-0.5 h-1.5 w-full overflow-hidden rounded-full bg-ash-900/[0.06]">
      ...
      {report.lifestyle.narrative && (
        <div className="rounded-lg bg-ash-900/[0.03] p-3">
          <p className="text-[12px] leading-snug text-ash-600">{report.lifestyle.narrative}</p>
          {report.lifestyle.tags.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1">
              {report.lifestyle.tags.slice(0, 6).map((t) => (
                <span key={t} className="rounded-full bg-paper px-2 py-0.5 text-[10px] font-medium text-ash-500 ring-1 ring-ash-900/5">{t}</span>
              ))}
            </div>
          )}
        </div>
      ...
      {report.loanBehaviour.lenders.length > 0 && (
        <div>
          <p className="mb-1.5 flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-ash-400">
          ...
          <div className="space-y-1">
            {report.loanBehaviour.lenders.slice(0, 4).map((l) => (
              <div key={l.name} className="flex items-center justify-between rounded-lg bg-paper/60 px-2.5 py-1.5 text-[11px] ring-1 ring-ash-900/5">
                <span className="font-medium text-ash-700">{l.name}</span>
                <span className="tabular-nums text-ash-500">borrowed {kes(l.borrowed)} · repaid {kes(l.repaid)}</span>
              </div>
            ))}
          </div>
          <p className="mt-1 text-[10px] text-ash-400">Repayment cadence: {report.loanBehaviour.repaymentCadence}.</p>
        </div>
      )}
