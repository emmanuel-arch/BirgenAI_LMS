...
export default function PhoneFloor({ extensions, cdr }: { extensions: Ext[]; cdr: Cdr[] }) {
  ...
  return (
    <div className="mx-auto max-w-[1300px] px-4 py-6 sm:px-6">
              ...
              {extensions.map((e) => (
                <div
                  ...
                  className={`flex items-center gap-2.5 rounded-lg border px-2.5 py-2 ${
                    e.agentName ? "border-ash-900/[0.08] bg-paper" : "border-dashed border-ash-900/12 bg-ash-900/[0.02]"
                  }`}
                >
                  <span
                    className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-[11px] font-bold tabular-nums ${
                      e.agentName ? "bg-[color:var(--accent)]/12 text-[color:var(--accent)]" : "bg-ash-900/[0.05] text-ash-400"
                    }`}
                  >
                    {e.extension || "—"}
                  </span>
                  <span className="min-w-0">
                    <span className={`block truncate text-[12px] font-medium ${e.agentName ? "text-ash-800" : "text-ash-400"}`}>
                      {e.agentName ?? "Unassigned"}
                    </span>
                    <span className="block truncate font-mono text-[9.5px] text-ash-400">{e.mac || "no MAC on file"}</span>
                  </span>
                </div>
              ))}
            </div>
              ...
              <table className="w-full min-w-[520px] text-left">
                <thead>
                  <tr className="text-[9.5px] font-bold uppercase tracking-[0.1em] text-ash-400">
                    <th className="sticky top-0 bg-paper px-2 pb-1.5">When</th>
                    <th className="sticky top-0 bg-paper px-2 pb-1.5">From → to</th>
                    <th className="sticky top-0 bg-paper px-2 pb-1.5">Talk</th>
                    <th className="sticky top-0 bg-paper px-2 pb-1.5">Status</th>
                  </tr>
                </thead>
                <tbody>
                  {cdr.map((c) => (
                    <tr key={c.id} className="border-t border-ash-900/[0.045]">
                      <td className="px-2 py-1.5 text-[10.5px] tabular-nums text-ash-500">
                        {c.start ? new Date(c.start).toLocaleDateString("en-KE", { day: "numeric", month: "short", year: "2-digit" }) : "—"}
                        <span className="block text-[9.5px] text-ash-400">{shortTime(c.start)}</span>
                      </td>
                      <td className="px-2 py-1.5">
                        <span className="block truncate text-[11px] tabular-nums text-ash-700">{c.from || "—"}</span>
                        <span className="block truncate text-[10px] tabular-nums text-ash-400">→ {c.to || "—"}</span>
                      </td>
                      <td className="px-2 py-1.5 text-[11px] tabular-nums text-ash-600">{c.talk || c.duration || "—"}</td>
                      <td className="px-2 py-1.5">
                        <span className="flex items-center gap-1">
                          <Tag tone={/answer|connect/i.test(c.status) ? "good" : "neutral"}>{c.status || c.type || "—"}</Tag>
                          {c.hasRecording && <Mic className="h-3 w-3 text-ash-400" aria-label="recorded" />}
                        </span>
                      </td>
                    </tr>
                  ))}
      ...
      <p className="mt-3 flex items-center gap-1.5 text-[10.5px] text-ash-400">
