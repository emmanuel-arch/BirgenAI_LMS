  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass p-5">
        <p className="text-sm font-semibold flex items-center gap-1.5"><UploadCloud className="h-4 w-4 text-ash-400" /> Logo</p>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-4">
          <button
            ...
            className="flex h-20 w-20 items-center justify-center rounded-xl border-2 border-dashed border-ash-900/15 bg-paper/60 hover:bg-paper"
          ...
          >
            {currentLogo ? (
              ...
              <UploadCloud className="h-6 w-6 text-ash-400" />
            )}
          </button>
          <div className="text-[11px] text-ash-500 space-y-1">
            ...
            <div className="mt-3 flex items-center gap-4">
              {[{ label: "Original", src: rawLogo }, { label: "Background removed", src: removedPreview }].map((v) => (
                <div key={v.label} className="text-center">
                  <div className="flex h-20 w-20 items-center justify-center rounded-lg border border-ash-900/10" style={CHECKER}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={v.src} alt={v.label} className="max-h-16 max-w-16 object-contain" />
                  </div>
                  <p className="mt-1 text-[10px] text-ash-500">{v.label}</p>
                </div>
              ))}
              <div className="flex flex-col gap-2">
                ...
                <button type="button" onClick={() => setRemovedPreview(null)} className="rounded-lg border border-ash-900/15 bg-paper/70 px-3 py-1.5 text-xs font-medium text-ash-600">
                  Keep the original
                </button>
              </div>
            </div>
        ...
        {note && <p className="mt-3 text-[11px] text-ash-500">{note}</p>}
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-semibold text-ash-600">Logo size</span>
            <code className="text-[11px] text-ash-500">{logoScale}%</code>
          </div>
      ...
      {/* Colors */}
      <div className="glass p-5">
        <p className="text-sm font-semibold flex items-center gap-1.5"><PaletteIcon className="h-4 w-4 text-ash-400" /> Colors</p>
        {palette.length > 0 && (
          <div className="mt-3">
            <p className="text-[11px] text-ash-500">From your logo — click a swatch to make it your accent:</p>
            <div className="mt-1.5 flex gap-2">
              {palette.map((p) => (
                <button
                  ...
                  className={`h-9 w-9 rounded-lg border-2 ${accent === p.color ? "border-ash-900" : "border-transparent"}`}
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <label className="block">
            <span className="text-[11px] font-semibold text-ash-600">Accent</span>
            <span className="block text-[11px] text-ash-500">The color of your buttons, links, highlights and active menu items.</span>
            <span className="mt-1.5 flex items-center gap-2">
              <input type="color" value={isHexColor(accent) ? accent : "#f97316"} onChange={(e) => pickAccent(e.target.value)} className="h-9 w-12 cursor-pointer rounded border border-ash-900/15" />
              <code className="text-xs text-ash-500">{accent}</code>
            </span>
          </label>
          <label className="block">
            <span className="text-[11px] font-semibold text-ash-600">Gradient partner</span>
            <span className="block text-[11px] text-ash-500">The far end of your gradient — used on your portal hero and sign-in page.</span>
            <span className="mt-1.5 flex items-center gap-2">
              <input type="color" value={isHexColor(accent2) ? accent2 : darken(accent)} onChange={(e) => { setAccent2(e.target.value); emit({ accent2: e.target.value }); }} className="h-9 w-12 cursor-pointer rounded border border-ash-900/15" />
              <code className="text-xs text-ash-500">{accent2}</code>
            </span>
          </label>
        </div>
      </div>
      ...
      <div className="glass p-5">
        ...
        <p className="mt-0.5 text-[11px] text-ash-500">Exactly what these settings drive — your console top bar, buttons, and portal hero.</p>
        <div className="mt-3 overflow-hidden rounded-xl border border-ash-900/10 bg-paper">
          <div className="flex min-h-11 items-center gap-2 border-b border-ash-900/10 px-3 py-1.5">
            {currentLogo ? (
            ...
            <span className="text-xs font-bold text-ash-800">{orgName}</span>
        ...
        {rawLogo && (
          <button type="button" onClick={() => { setRawLogo(null); setLogo(null); setPalette([]); setTransparent(null); setRemovedPreview(null); emit({ logo: null }); }} className="mt-3 inline-flex items-center gap-1.5 text-[11px] text-ash-500 hover:text-ash-800">
