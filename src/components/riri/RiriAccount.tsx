...
function Section({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.12em] text-ash-400">
        {icon} {title}
      </p>
...
export function RiriAccount({
  ...
  if (!acc) return <p className="flex items-center gap-2 px-4 py-6 text-xs text-ash-500"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Loading your account…</p>;

  const toggle = "flex w-full items-center justify-between rounded-xl border border-ash-900/10 bg-paper/70 px-3 py-2.5 text-left";
  const u = acc.usage;
  ...
  return (
    <div className={inline ? "space-y-4 px-4" : "flex-1 min-h-0 space-y-4 overflow-y-auto px-4 py-3"}>
      <Section icon={<UserRound className="h-3 w-3" />} title="Who I know you as">
        <div className="rounded-xl border border-ash-900/10 bg-paper/70 px-3 py-2.5">
          <p className="text-[13px] font-bold text-ash-800">{acc.actor.name ?? "—"}</p>
      ...
      <Section icon={<Gauge className="h-3 w-3" />} title={`Your usage — ${u?.monthLabel ?? "this month"}`}>
        <div className="grid grid-cols-4 gap-1.5">
          ...
          {([
            ...
            <div key={label} className="rounded-xl border border-ash-900/10 bg-paper/70 px-2 py-2 text-center">
      ...
      <Section icon={<Brain className="h-3 w-3" />} title="What I remember about you">
        {acc.memories.length === 0 ? (
          <p className="rounded-xl border border-dashed border-ash-900/15 px-3 py-3 text-[11px] text-ash-500">
          ...
          <div className="space-y-1.5">
            {acc.memories.map((m) => (
              <div key={m.id} className="flex items-start justify-between gap-2 rounded-xl border border-ash-900/10 bg-paper/70 px-3 py-2">
                <div className="min-w-0">
                  <p className="text-[9px] font-bold uppercase tracking-wide text-ash-400">
                    {KIND_LABEL[m.kind] ?? m.kind} · {new Date(m.createdAt).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}
                  </p>
                  <p className="mt-0.5 text-[11px] leading-snug text-ash-700">{m.body}</p>
                </div>
                <button
                  onClick={() => forget(m.id)}
                  disabled={!!busyId}
                  className="shrink-0 rounded-lg p-1 text-ash-300 hover:bg-rose-50 hover:text-rose-600 disabled:opacity-40"
      ...
      <Section icon={<Settings2 className="h-3 w-3" />} title="Settings">
        <div className="space-y-1.5">
          <button onClick={onVoice} className={toggle}>
            <span className="flex items-center gap-2 text-[12px] font-medium text-ash-700">
              {voiceOn ? <Volume2 className="h-3.5 w-3.5" style={{ color: "var(--brand)" }} /> : <VolumeX className="h-3.5 w-3.5 text-ash-400" />}
              Speak answers out loud
            </span>
            <span className={`text-[10px] font-bold ${voiceOn ? "text-[color:var(--brand)]" : "text-ash-400"}`}>
          ...
          <button onClick={onAutoGo} className={toggle} title={`When on, ${ASSISTANT_NAME} takes you straight to the screen she suggests`}>
            <span className="flex items-center gap-2 text-[12px] font-medium text-ash-700">
              <ArrowRight className={`h-3.5 w-3.5 ${autoGo ? "" : "text-ash-400"}`} style={autoGo ? { color: "var(--brand)" } : undefined} />
              Take me to screens she suggests
            </span>
            <span className={`text-[10px] font-bold ${autoGo ? "text-[color:var(--brand)]" : "text-ash-400"}`}>{autoGo ? "ON" : "OFF"}</span>
          </button>
          <button onClick={onLang} className={toggle}>
            <span className="text-[12px] font-medium text-ash-700">Voice language</span>
            <span className="text-[10px] font-bold" style={{ color: "var(--brand)" }}>{lang === "sw-KE" ? "Kiswahili" : "English"}</span>
          </button>
        </div>
        <p className="mt-2 text-[10px] leading-snug text-ash-400">
          {acc.llm === "live"
