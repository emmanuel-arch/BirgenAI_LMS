  ...
  return createPortal(
    ...
    >
      <motion.div
        ...
        className={`flex max-h-[88dvh] w-full flex-col overflow-hidden rounded-3xl border border-ash-900/10 bg-paper shadow-2xl ${WIDTH[width]}`}
      >
        <div className="flex shrink-0 items-start justify-between gap-3 px-5 pt-5">
          <div className="min-w-0">
            <h2 className="text-base font-bold">{title}</h2>
            {sub && <p className="mt-0.5 text-xs text-ash-500">{sub}</p>}
          </div>
          <button onClick={onClose} aria-label="Close"
            className="shrink-0 rounded-lg p-1.5 text-ash-400 hover:bg-ash-900/5 hover:text-ash-700">
            <X className="h-4 w-4" />
          </button>
        </div>
        {subheader && <div className="shrink-0 px-5 pt-3">{subheader}</div>}
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-5">{children}</div>
        {footer && <div className="shrink-0 border-t border-ash-900/10 px-5 py-4">{footer}</div>}
      </motion.div>
    </motion.div>,
    document.body,
  );
