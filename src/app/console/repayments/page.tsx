...
export default function RepaymentsPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 sm:px-6 py-8">
        ...
        {!loans && !error && <div className="mt-10 flex justify-center"><Loader2 className="h-5 w-5 animate-spin text-ash-400" /></div>}
                ...
                <div key={r.id} className="glass p-4 border-amber-200">
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    ...
                    <button onClick={() => { setAllocFor(allocFor === r.id ? null : r.id); setAllocLoan(""); }}
                      className="rounded-lg border border-ash-900/15 bg-paper/70 px-3 py-1.5 text-xs font-semibold text-ash-700 hover:bg-paper">
                      Allocate to loan
                    </button>
                  </div>
                  {allocFor === r.id && (
                    <div className="mt-3 flex flex-wrap gap-2">
                      <select value={allocLoan} onChange={(e) => setAllocLoan(e.target.value)}
                        className="rounded-lg border border-ash-900/15 bg-paper/80 px-3 py-2 text-sm outline-none flex-1 min-w-56">
        ...
        <section className="mt-6">
          <h2 className="text-sm font-semibold text-ash-700">Active loans ({loans?.length ?? 0})</h2>
          <div className="mt-2 space-y-2">
            {loans?.map((l) => (
              <div key={l.id} className="glass p-4 flex items-center justify-between gap-3 flex-wrap">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-ash-500">REF {l.ref}{l.nextDue ? ` · next ${fmtKES(l.nextDue.amount)} due ${l.nextDue.date}` : ""}</p>
                </div>
            ...
            {loans?.length === 0 && <p className="text-sm text-ash-500">No active loans.</p>}
          </div>
        </section>

        <section className="mt-6">
          <h2 className="text-sm font-semibold text-ash-700">Recent receipts & STK requests</h2>
          <div className="mt-2 space-y-2">
            {receipts.filter((r) => r.allocatedLoanId).slice(0, 10).map((r) => (
              <div key={r.id} className="glass p-3 flex items-center justify-between gap-3">
                <p className="text-xs text-ash-600 truncate">{r.transId} · {fmtKES(r.amount)} {r.phone ? `· ${r.phone}` : ""} · {new Date(r.createdAt).toLocaleString("en-KE", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}</p>
            ...
            {intents.slice(0, 10).map((i) => (
              <div key={i.id} className="glass p-3 flex items-center justify-between gap-3">
                <p className="text-xs text-ash-600 truncate">STK · {fmtKES(i.amount)} · {i.phone} {i.mpesaReceipt ? `· ${i.mpesaReceipt}` : ""} {i.resultDesc ? `· ${i.resultDesc}` : ""}</p>
