    ...
    <div className="min-h-screen rounded-2xl bg-paper text-ash-900 print-doc">
      ...
      <div className="no-print sticky top-0 z-10 rounded-t-2xl border-b border-ash-900/10 bg-paper/80 backdrop-blur">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 h-14 flex items-center justify-between">
          <Link href={`/console/borrowers/${loan.borrower.id}`} className="inline-flex items-center gap-1.5 text-sm text-ash-500 hover:text-ash-800">
      ...
      <main className="mx-auto max-w-3xl px-4 sm:px-6 py-8 print-exact">
        ...
        <section className="mt-5 grid grid-cols-2 gap-6 text-[12px] print-break">
          ...
          <div>
            ...
            <p className="text-ash-600">Status {loan.status} · disbursed {d(loan.disbursedAt)}</p>
        ...
        <section className="mt-5 grid grid-cols-2 sm:grid-cols-5 gap-2 print-break">
          {[
            ...
            <div key={t.l} className="rounded-lg border border-ash-900/10 px-2.5 py-2">
              ...
              <p className={`text-sm font-bold ${i === 4 ? "" : "text-ash-800"}`} style={i === 4 ? { color: org?.accent } : undefined}>{t.v}</p>
            </div>
          ))}
        </section>
            ...
            <tbody>
              {loan.installments.map((i) => (
                <tr key={i.id} className="border-b border-ash-900/5">
                  ...
                  <td className={`py-1.5 text-right font-semibold ${INST_TONE[i.status] ?? "text-ash-500"}`}>{i.status}</td>
                </tr>
              ))}
