...
export default function DemoPage() {
  ...
  return (
    <div className="min-h-screen relative text-ash-900" style={{ ["--brand" as never]: "#6d28d9", ["--brand-soft" as never]: "rgba(109,40,217,0.12)" }}>
      ...
      <main className="relative z-10 mx-auto max-w-5xl px-4 sm:px-6 py-10">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="mt-3 max-w-2xl text-ash-600">
          ...
          <p className="mt-2 text-xs text-ash-400">Shared password <button onClick={() => copy(PASSWORD, "pw")} className="font-mono font-semibold text-ash-600 underline">{PASSWORD}</button> {copied === "pw" && "✓ copied"}</p>
        </motion.div>

        {error && <div className="mt-4 rounded-lg border border-red-300 bg-red-50/90 px-3 py-2.5 text-sm text-red-700">{error}</div>}

        {/* Staff roles */}
        <h2 className="mt-9 flex items-center gap-2 text-sm font-bold uppercase tracking-wide text-ash-500"><Users className="h-4 w-4" /> Staff — sign in as a role</h2>
            ...
            <motion.div key={r.email} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-ash-500 truncate">{r.name}</p>
                </div>
              </div>
              <p className="mt-2.5 flex-1 text-xs leading-relaxed text-ash-600">{r.shows}</p>
              <div className="mt-3 flex items-center gap-2">
                ...
                <button onClick={() => copy(r.email, r.email)} className="rounded-lg border border-ash-900/15 bg-paper/70 p-2 text-ash-500 hover:bg-paper" title="Copy email">
        ...
        {/* Borrower journeys */}
        <h2 className="mt-9 flex items-center gap-2 text-sm font-bold uppercase tracking-wide text-ash-500"><UserRound className="h-4 w-4" /> Borrowers — walk the customer journeys</h2>
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          {BORROWERS.map((b) => (
            <div key={b.phone} className="glass p-4">
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm font-bold">{b.name}</p>
                <span className="rounded-md bg-ash-900/5 px-2 py-0.5 text-[10px] font-semibold text-ash-600">{b.tag}</span>
              </div>
              <p className="mt-1 text-[11px] text-ash-500">Phone {b.phone} · ID {b.nid}</p>
