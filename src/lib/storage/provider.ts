...
/**
 * Store a generated report (Riri Analytics exports).
 *
 * Lands in the PRIVATE docs bucket under the org's own prefix, so a report is reachable
 * only through a signed URL and only for the tenant that asked for it. The filename is
 * the autogenerated, human-readable one (RIRI_PAR30_BY_PRODUCT_2026-07-16.xlsx) rather
 * than a uuid: this is a file a manager will forward, and "download (3).xlsx" is not
 * something anyone can act on. Uniqueness comes from the timestamped folder above it,
 * so two runs of the same question never overwrite each other.
 */
export async function putReportObject(
  ...
  return putObject(DOCS_BUCKET, `${orgId}/reports/${Date.now()}-${randomUUID().slice(0, 8)}/${safe}`, buffer, contentType);
}
