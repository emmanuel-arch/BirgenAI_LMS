...
export const SYSTEM_SCREENS: SystemScreen[] = [
  ...
  {
    id: "riri-knowledge",
    href: "/console/settings/riri",
    title: "Riri Knowledge",
    module: "assistant",
    purpose:
      "Train Riri on your own facts — products, fees, terms, contact — so she answers your customers first, from your words, with your pack id on every answer.",
    does: [
      "State your KRA PIN, which owns every fact you publish",
      "Upload a knowledge pack as JSON and see every problem with it before it goes live",
      "Ask a test question and see exactly what Riri would tell a customer from the draft",
      "Publish it as a new version, or roll back to an earlier one",
      "Switch Riri's first response for customers on or off",
    ],
    asks: [
      "riri knowledge", "train riri", "teach riri", "upload a knowledge pack", "knowledge pack", "riri training",
      "what riri tells customers", "customer faq", "change riri's answers", "riri fees answer", "white label riri",
      "fundisha riri", "maarifa ya riri",
    ],
    concepts: ["knowledge pack", "tenant pack", "pack version", "first response"],
    implications: [
      "A pack states facts about YOU. It cannot describe the software — an entry that says 'go to Settings' or links to an app route is refused, because those screens change without you.",
      "Every answer a customer gets from your pack names the pack and its version. When a fee changes, publish a new version — the old answers stop the same minute.",
      "Until you publish, your customers are answered from a STARTER pack we assembled from your published terms, and every one of those answers says so.",
      "Turning first response off sends customers straight to the team — every question, including the ones Riri answers in a second.",
    ],
    right: "settings.view",
    related: ["conversations", "credit-policy"],
  },
  {
    id: "conversations",
    ...
    module: "borrowers",
    purpose:
      "The queue of customers waiting on a reply — including every case Riri handed over, arriving with what they asked, what she checked and the one thing you need to decide.",
    does: [
      "Work the queue oldest-waiting first",
      "Read Riri's triage note before replying",
      "Reply, assign, or resolve a conversation",
      "See what Riri answered without reaching you, and what she hands over most",
    ],
    asks: [
      "customer messages", "conversations", "inbox", "customer inbox", "reply to a customer", "message queue",
      "escalations", "riri escalations", "what did riri hand over", "cases from riri", "ujumbe wa wateja",
    ],
    concepts: ["first response", "triage note", "deflection"],
    implications: [
      "A thread Riri hands over is already the team's move — it sits in 'Needs a reply' exactly as if the customer had typed it.",
      "Replying marks the thread 'Waiting on them'. The customer writing again reopens it; only a person resolves it.",
      "The questions Riri hands over most are the next knowledge entries worth writing in Riri Knowledge.",
    ],
    right: "borrowers.view",
    related: ["riri-knowledge", "borrowers-list"],
  },
  {
    id: "loan-settings",
    href: "/console/settings/loans",
    title: "Loan Settings",
    module: "organization",
    purpose: "How loans behave once written — the loan-level rules and fields this lender needs that the platform does not ship by default.",
    does: ["Set loan-level rules", "Add the loan fields this lender needs", "Publish a new version of the loan settings"],
    asks: ["loan settings", "loan rules", "loan configuration", "loan fields", "mipangilio ya mikopo"],
    implications: ["Published settings apply to loans written after the publish, not retroactively."],
    right: "settings.view",
    related: ["products", "credit-policy"],
  },
  {
    id: "attachment-types",
    href: "/console/settings/attachments",
    title: "Attachment Types",
    module: "organization",
    purpose: "The documents this lender collects — which are required, what file types are accepted, and how large they may be.",
    does: ["Add or remove a document type", "Mark a document as required", "Set the accepted file types and size"],
    asks: ["attachment types", "document types", "required documents", "what documents do we collect", "aina za viambatisho"],
    implications: ["A required document blocks onboarding in the customer app until it is uploaded."],
    right: "settings.view",
    related: ["borrower-settings"],
  },
  {
    id: "identity-policy",
    ...
    module: "organization",
    purpose: "Where the machine stops and a person starts on every identity check — the face-match bands, liveness, and what gets referred for review.",
    does: ["Set the face-match thresholds", "Choose which signals refer a check to a person", "Decide whether every new customer is reviewed"],
    asks: ["identity policy", "kyc policy", "face match threshold", "liveness settings", "manual review", "sera ya utambulisho"],
    implications: [
      "It governs every FUTURE check, not the ones already in the queue.",
      "Narrowing the review band sends fewer checks to a person — and puts more weight on a reader that simulates when its engine is unreachable.",
    ],
    right: "settings.view",
    related: ["kyc-queue", "borrower-settings"],
  },
  {
    id: "additional-details",
    href: "/console/settings/details",
    title: "Additional Details",
    module: "organization",
    purpose: "The lender's own extra questions at onboarding — business type, income source, anything the standard fields do not ask.",
    does: ["Add a question and its answer type", "Group questions", "Mark a question as required"],
    asks: ["additional details", "custom fields", "extra questions", "onboarding questions", "maswali ya ziada"],
    implications: ["A required detail is asked of every new customer in the app before they can finish signing up."],
    right: "settings.view",
    related: ["borrower-settings", "attachment-types"],
  },
  {
